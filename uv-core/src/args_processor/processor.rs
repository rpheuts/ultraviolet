...
mod tests {
    ...
    fn test_process_args() {
        let processor = ArgsProcessor::new(create_test_schema());
        ...
        let result = processor.process_args(&args).unwrap();
        
        assert_eq!(result["name"], "test");
        assert_eq!(result["count"], 42);
        assert_eq!(result["active"], true);
        assert_eq!(result["tags"], json!(["one", "two"]));
        assert_eq!(result["with_default"], "default_value");
    }
}
